...
ggplot(data = diamonds_sample) +
  geom_col(mapping = aes(x = cut, y = price), position = "dodge")
...
# for faster results)
ggplot(data = diamonds_sample) +
  geom_boxplot(mapping = aes(x = carat, y = price, color = clarity)) 
...
ggplot(data = diamonds_sample) +
  geom_col(mapping = aes(x = cut, fill = x, y = price), width = 1)

# Draw the same chart, but with the coordinate system flipped
ggplot(data = diamonds_sample) +
  geom_col(mapping = aes(y = cut, fill = y, x = price), width = 1)

# Draw the same chart, but in a polar coordinate system. It's a Coxcomb chart!
polar <- ggplot(data = diamonds_sample) +
  geom_col(mapping = aes(x = cut, fill = x, y = price), width = 1) 
polar + coord_polar()
